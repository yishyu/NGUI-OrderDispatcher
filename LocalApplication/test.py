...
def correct_skew(image):
    ...
    while True:
        ...
        osd_rotated_image = pytesseract.image_to_osd(otsu, config='--psm 0 -c min_characters_to_try=5')
...
no_shadows = de_shadow(frame)
# no_shadows = remove_shadows(frame)

# correcting the 2d skew of the image
deskewed = correct_skew(no_shadows)
deskewed_copy = deskewed.copy()

# applying gaussian blur to reduce noice(little bit)
gaussian_blur = cv2.GaussianBlur(deskewed,(5,5),0)

# sharpening the image to preserve edges due to blurring
...
# setting the ocr-engine-mode(oem) and page-segmentation-mode(psm) to 1, 12 resp. for tesseract
custom_oem_psm_config = r'--oem 1 --psm 12 -c min_characters_to_try=0'

# passing the binarized image to tesseract's image_to_data() function
...
for i in range(boxes):
    if (int(ocr['conf'][i])>64):
        ...
        cv2.rectangle(deskewed_copy,(x,y),(x+w,y+h),(255,0,0),2)
        #ocr['text'][i] = spell.correction(ocr['text'][i])
        texts.append(ocr['text'][i])
        #cv2.imshow("text",deskewed_copy)
...
cv2.imwrite("text.jpg",deskewed_copy)
...
# writing the text into a text file.
